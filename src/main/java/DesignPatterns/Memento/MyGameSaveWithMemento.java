...
class GameRole {
    ...
    public void setDefense(int value){ this.defense = value; }
    //状态显示
    ...
    //保存角色状态
    public RoleStateMemento saveState(){ return new RoleStateMemento(this.vitality,this.attack,this.defense); }  //将状态传入备忘录，并返回该备忘录实例
    //恢复角色状态
...
//主程序入口
public class MyGameSaveWithMemento {
    public static void main(String[] args){
        //大战Boss前
        GameRole role = new GameRole();
        role.getInitState(); //初始化状态
        role.displayState(); //显示状态

        //保存进度
        RoleStateCaretaker stateAdmin = new RoleStateCaretaker();  //管理者
        stateAdmin.setRoleStateMemento(role.saveState()); //生成备忘录，并将其发给管理者
        ...
        //游戏进度恢复
        role.recoveryState(stateAdmin.getRoleStateMemento());  //从管理者手中获得备忘录，并读取其中数据恢复自己
        role.displayState(); //显示状态
    }
}