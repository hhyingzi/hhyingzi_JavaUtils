...
class ConcreteCompany extends Company{
    ...
    public void display(int depth) {  //深度优先遍历
        for(var i=0;i<depth;i++) System.out.print("-");
        System.out.println(name);
        for(Company item : children) item.display(depth+2);
    }
    //履行职责，叶子节点独有
    public void lineOfDuty(){
        for(Company item : children) item.lineOfDuty();
    }
}
//人力资源部，树叶节点
class HRDepartment extends Company{
    public HRDepartment(String name){ super(name); }
    public void add(Company company){ }  //叶子节点没有 add()
    public void remove(Company company){ }  //叶子节点没有 remove()
...
class FinanceDepartment extends Company{
    ...
    //履行职责
    public void lineOfDuty(){ System.out.println(name+" 公司财务收支管理"); }
}
//主程序入口
public class MyCompanySystemWithComposite {
